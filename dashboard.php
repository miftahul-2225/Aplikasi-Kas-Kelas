...
<head>
    ...
    <style>
        ...
        /* ══ MAIN ══ */
        #main {
            margin-left: var(--sb-full); min-height: 100vh;
            padding: 28px; transition: margin-left var(--ease);
        }
        #main.expanded { margin-left: var(--sb-mini); }

        /* ══ CHART ══ */
        .chart-wrapper { position: relative; width: 180px; height: 180px; margin: 0 auto; }
        .chart-center-label {
            position: absolute; top: 50%; left: 50%;
            transform: translate(-50%, -50%);
            text-align: center; pointer-events: none;
        }
        .chart-center-label .pct { font-size: 22px; font-weight: 700; color: #0d6efd; line-height: 1; }
        .chart-center-label .lbl { font-size: 10px; color: #888; }
        .legend-dot { width: 10px; height: 10px; border-radius: 50%; display: inline-block; margin-right: 4px; }

        /* ══ ANIMASI ══ */
        .card-anim { opacity: 0; transform: translateY(18px); animation: fadeUp 0.5s ease forwards; }
...
<main id="main">
    ...
    <div class="row g-4 align-items-start">
            ...
            <div class="card border rounded-4 p-3 card-anim" style="animation-delay:0.4s;">
                ...
                <div class="d-flex justify-content-center gap-4 mt-3">
                    <div class="text-center">
                        ...
                        <div class="fw-bold small text-success">Rp <?= number_format($pemasukan, 0, ',', '.') ?></div>
                    </div>
                    <div class="text-center">
                        <div class="small text-muted mb-1">
                            <span class="legend-dot" style="background:#dc3545;"></span>Pengeluaran
                        </div>
                        <div class="fw-bold small text-danger">Rp <?= number_format($pengeluaran, 0, ',', '.') ?></div>
                    </div>
                </div>
                <hr class="my-2">
                <p class="text-center small mb-0 text-muted">
                    Saldo bersih: <strong class="text-primary">Rp <?= number_format($saldo, 0, ',', '.') ?></strong>
                </p>
            </div>
        </div>
        ...
        <div class="col-12 col-md-8">
            ...
            <?php $delay = 0.5; while ($row = mysqli_fetch_assoc($transaksi)) : ?>
                    ...
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <p class="fw-semibold small mb-0">
                                <?= $row['jenis'] == 'bayar' ? 'Iuran Kas - ' . htmlspecialchars($row['nama_siswa']) : 'Pengeluaran Kas' ?>
                            </p>
...
<script>
    function animateCounter(elementId, targetValue, duration = 1500) {
        ...
        function update(currentTime) {
            ...
            const current = Math.round(targetValue * easeOutQuart(progress));
            el.textContent = 'Rp ' + current.toLocaleString('id-ID');
            if (progress < 1) requestAnimationFrame(update);
            else el.textContent = 'Rp ' + targetValue.toLocaleString('id-ID');
        }
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{ data: [<?= (int)$pemasukan ?>, <?= (int)$pengeluaran ?>], backgroundColor: ['#198754', '#dc3545'], borderWidth: 0, hoverOffset: 8 }]
        },
        options: {
            cutout: '72%',
